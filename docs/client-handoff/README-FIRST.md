...
| Source monorepo commit | `aa1e12b2d0027d96bce208a845e2f96ec02c7ec2` |
| Machine baseline | `client-contract-baseline.json` |
| Current handoff | `CONTRACT-2.0.10-HANDOFF.md` |

开发前依次核验 `CONTRACT-2.0.10-HANDOFF.md`、`client-contract-baseline.json`、权威 OpenAPI 字节和 SHA-256。Android 与 Web 快照必须与权威 OpenAPI byte-identical；它们绑定到上表所示 release state，公开分发前仍须验证 GitHub Release 资产。当前没有已确认的权威 iOS 工程，导入真实工程后必须从正式 Release 资产导入相同字节，并在 iOS CI 固定相同 version/hash。
