...
| Source monorepo commit | `776e30419e522a873534d5bafba94797616c5021` |
| Machine baseline | `client-contract-baseline.json` |
| Current handoff | `CONTRACT-2.0.12-HANDOFF.md` |

开发前依次核验 `CONTRACT-2.0.12-HANDOFF.md`、`client-contract-baseline.json`、权威 OpenAPI 字节和 SHA-256。Android 与 Web 快照必须与权威 OpenAPI byte-identical；它们已绑定到正式 Release，但公开分发前仍须验证 GitHub Release 资产。当前没有已确认的权威 iOS 工程；导入真实工程后必须从正式 Release 资产导入相同字节，并在 iOS CI 固定相同 version/hash。
